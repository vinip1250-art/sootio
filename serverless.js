...
    const config = parseConfiguration(req.params.configuration)
    // Pega o objeto 'extra' da URL (contém filtros como 'language:pt')
    let extra = req.params.extra ? qs.parse(req.url.split('/').pop().slice(0, -5)) : {}
    
    // INÍCIO DA CORREÇÃO PARA FILTRO DE IDIOMA ABRANGENTE
    if (extra.language && extra.language.toLowerCase() === 'pt') {
        const languageTerms = 'brazilian portugues';
        
        // Adiciona os termos ao filtro de busca existente ou o cria.
        if (extra.search) {
            extra.search = `${extra.search} ${languageTerms}`;
        } else {
            extra.search = languageTerms;
        }
        
        // Remove o filtro de idioma 'pt' para evitar que o SDK o use.
        delete extra.language;
        
        console.log(`[LANGUAGE-FILTER] Expanded 'pt' filter to search for: ${extra.search}`);
    }
    // FIM DA CORREÇÃO PARA FILTRO DE IDIOMA
    
    const host = `${req.protocol}://${req.headers.host}`;
    const clientIp = requestIp.getClientIp(req);

    // Combine todos os valores de configuração corretamente, incluindo clientIp
